#!/usr/bin/env python3
"""Script to test all noise schedules for CT (Continuous-Time) model and find the best based on Chamfer distance."""
    ...
    if chamfer_dist is None:
        ...
        if artifacts_dir.exists():
            ...
            for dir_path in dirs:
                ct_dir = dir_path / 'ct'
                results_file = ct_dir / 'training_results.pkl'
...
def main():
    ...
    print("="*60)
    print("Testing all noise schedules for CT (Continuous-Time) model")
    print("="*60)
    print(f"Schedules to test: {', '.join(NOISE_SCHEDULES)}")
    print(f"\nConfig:")
    print(f"  - Model: CT (Continuous-Time)")
