...
class PosEmbMLPSwinv1D(nn.Module):
    ...
    @nn.compact
    def __call__(self, x: jnp.ndarray, dim: int):
        ...
        if self.rank == 1:
            ...
            if self.conv:
                ...
                pos_emb = nn.Dense(512, use_bias=True)(relative_coords_table)
                pos_emb = nn.relu(pos_emb)
                pos_emb = nn.Dense(dim, use_bias=False)(pos_emb)
        else:
            ...
            if self.conv:
                ...
                pos_emb = nn.Dense(512, use_bias=True)(flattened_table)
                pos_emb = nn.relu(pos_emb)
                pos_emb = nn.Dense(dim, use_bias=False)(pos_emb)
...
class PosEmbMLPSwinv2D(nn.Module):
    """
    From FasterViT via jimmy, for Windowed Attention
    """
    ...
    def __call__(self, x: jnp.ndarray, local_window_size: int):
        ...
        if self.pretrained_window_size > 0:
            relative_coords_table = relative_coords_table.at[:, :, :, 0].set(
                relative_coords_table[:, :, :, 0] / self.pretrained_window_size -
                1)
            relative_coords_table = relative_coords_table.at[:, :, :, 1].set(
                relative_coords_table[:, :, :, 1] / self.pretrained_window_size -
                1)
        else:
            relative_coords_table = relative_coords_table.at[:, :, :, 0].set(
        ...
        # Create the MLP layers
        cpb_mlp = nn.Dense(512, use_bias=True)(relative_coords_table)
        cpb_mlp = nn.relu(cpb_mlp)
        cpb_mlp = nn.Dense(self.num_heads, use_bias=False)(cpb_mlp)
        
        relative_position_bias_table = cpb_mlp.reshape(-1, self.num_heads)
        relative_position_bias = relative_position_bias_table[
            relative_position_index.reshape(-1)].reshape(
        ...
        pos_emb = relative_position_bias[jnp.newaxis, ...]
...
class RoPE(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray):
        ...
        rotations = jnp.stack([rotations_re, rotations_im], axis=-1)
        ...
        # Apply rotation
        rotations_complex = rotations[..., 0] + 1j * rotations[..., 1]
