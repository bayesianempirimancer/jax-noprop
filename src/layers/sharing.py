...
class LoRA(nn.Module):
    """
    Very simple implementation of a LoRA[1] layer from jimmy.
    ...
    @nn.compact
    def __call__(self, x: jnp.ndarray, in_features: int, out_features: int):
        x = nn.Dropout(self.drop_rate)(x, deterministic=False)

        x = nn.Dense(self.rank, use_bias=False)(x)
        x = nn.Dense(out_features, use_bias=False, kernel_init=nn.initializers.zeros)(x)

        return x * (self.alpha / self.rank)


class LayerSharing(nn.Module):
    """
    ...
    @nn.compact
    def __call__(self, x: jnp.ndarray, dim: int, f: nn.Module):
        if self.lora:
            loras = [LoRA(**self.lora_kwargs) for _ in range(self.repetitions)]
            
            def forward(i: int, x: jnp.ndarray):
                def apply_lora(j, x):
                    return loras[j](x, dim, dim)

                lora_output = switch(
                    i, [lambda x: apply_lora(j, x) for j in range(self.repetitions)],
                    x)
                return f(x) + lora_output
        else:
            def forward(i: int, x: jnp.ndarray):
                return f(x)

        x = fori_loop(0, self.repetitions, forward, x)
        return x
