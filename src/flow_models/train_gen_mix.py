...
from src.flow_models.fm_mix_trainer import train_epoch_fm_mix, save_results_fm_mix
from src.flow_models.config_mix import Config
from src.configs.base_config import BaseConfig
from src.flow_models.training_utils import get_save_directory
...
def main():
    ...
    parser.add_argument('--update_gmm', action='store_true', default=True,
                       help='Update GMM parameters during training (default: True)')
    parser.add_argument('--no_update_gmm', dest='update_gmm', action='store_false',
                       help='Disable GMM parameter updates during training')
    ...
    if is_config_mix and hasattr(base_config, 'override_from_args'):
        # Use config_mix override_from_args
    ...
    # Fit GMM if using mixture sampling
    sample_method = config.flow_planner.get('sample_method', 'mixture')
    if sample_method == "mixture" and args.update_gmm:
        print("\nFitting GMM to training data...")
        from src.vae.vb_gmm import GMMVBEM, GMMVBEMConfig
        
        # Encode training data to get z_target
        print("  Encoding training data to get z_target...")
        z_target_batch = []
        num_batches_for_gmm = min(10, (train_y.shape[0] + args.batch_size - 1) // args.batch_size)
        for i in range(num_batches_for_gmm):
            start_idx = i * args.batch_size
            end_idx = min(start_idx + args.batch_size, train_y.shape[0])
            y_batch = train_y[start_idx:end_idx]
            
            key, encode_key = jr.split(key)
            mu_z, logvar_z = model.apply(params, y_batch, method='encode', training=False, rngs={'dropout': encode_key})
            z_target = mu_z + jr.normal(jr.PRNGKey(i), mu_z.shape) * jnp.exp(0.5 * logvar_z)
            z_target_batch.append(z_target)
        
        z_target_all = jnp.concatenate(z_target_batch, axis=0)
        print(f"  Encoded {z_target_all.shape[0]} samples to latent space")
        
        # Get GMM config
        gmm_config_dict = config.flow_planner.get('gmm', {})
        gmm_config = GMMVBEMConfig(
            num_clusters=gmm_config_dict.get('num_clusters', 8),
            latent_dim=int(np.prod(config.main.get('latent_shape', (2,)))),
            ...
            beta_mix=gmm_config_dict.get('beta_mix', 1.0),
            tie_precisions=gmm_config_dict.get('shared_variances', False)
        )
        
        # Extract GMM params
        gmm_params_nested = params['params']['flow_planner']['gmm']
        temp_params = {'params': {'gmm': gmm_params_nested}}
        
        def planner_apply_fn(p, x_batch, **kwargs):
            """Apply function that calls the planner's gmm.update method."""
            model_params = {
                'params': {
                    'flow_planner': {'gmm': p['params']['gmm']},
                    'crn_model': params['params']['crn_model'],
                    'encoder': params['params'].get('encoder', {}),
                    'decoder': params['params'].get('decoder', {}),
                }
            }
            update_kwargs = {k: v for k, v in kwargs.items() if k != 'method'}
            updated_dict = model.apply(
                model_params,
                x_batch,
                method=lambda mdl, x, **kw: mdl.flow_planner.gmm.update(x, **kw),
                **update_kwargs
            )
            return updated_dict
        
        # Fit GMM
        updated_temp_params = GMMVBEM.fit(
            config=gmm_config,
            params=temp_params,
            x_data=z_target_all,
            apply_fn=planner_apply_fn,
            initialize=True,
            num_epochs=5,
            batch_size=args.batch_size,
            N_eff=float(z_target_all.shape[0]),
            lr=args.gmm_lr,
            seed=args.seed
        )
        
        # Update params with fitted GMM
        from flax.core import unfreeze, freeze
        params_unfrozen = unfreeze(params)
        gmm_params_updated = freeze(updated_temp_params['params']['gmm'])
        params_unfrozen['params']['flow_planner']['gmm'] = gmm_params_updated
        params = freeze(params_unfrozen)
        print("  GMM fitting complete!")
        
        # Reinitialize optimizer with updated params
        opt_state = optimizer.init(params)
    ...
    for epoch in range(args.num_epochs):
        ...
        update_gmm_epoch = (sample_method == "mixture") and args.update_gmm
