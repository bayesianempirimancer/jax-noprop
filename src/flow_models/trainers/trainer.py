"""
Minimal, professional JAX trainer for VAE flow models using unified FlowModel.
...
from src.flow_models.flow_model import FlowModel
...
class Trainer:
    ...
    def __init__(
        ...
        # Use unified FlowModel for all types
        # The specific behavior (flow/target/noise loss) is determined by config.loss_type
        self.model = FlowModel(config=config)
    ...
    def initialize(self, x_sample: Optional[jnp.ndarray], y_sample: jnp.ndarray):
        ...
        Args:
            x_sample: Sample input matching input_shape or [batch_size, *input_shape]. Can be None.
    ...
    def train_step(self, x_batch: jnp.ndarray, y_batch: jnp.ndarray, use_dropout: bool = True) -> Dict[str, float]:
        """
        Single training step using FlowModel's built-in train_step method.
        ...
        x_batch = jnp.asarray(x_batch) if x_batch is not None else None
    ...
    def train_epoch(
        ...
        for batch in dataloader:
            if x_data is not None:
                ...
                # Handle single array dataset output
    ...
    def train(
        ...
        for epoch in range(num_epochs):
            ...
            import numpy as np
            num_viz = min(2000, y_data.shape[0])
            # For PVE/MSE we need predictions.
            # For unconditional gen, predict() needs x=None?
            # self.predict signature: (self, params, x, ...)
            # If x_data is None, predict(params, None) works?
            # FlowModel.predict signature: (params, x, ...)
            # It calls predict_latent then decode.
            # predict_latent calls integrate_ode with x.
            # integrate_ode -> vector_field(z, x, t) -> dz_dt(z, x, t)
            # dz_dt handles x=None.
            
            # So if x_data is None, we pass None.
            if x_data is not None:
                x_viz = x_data[:num_viz]
            else:
                x_viz = None
                
            # But wait, predict() is usually for regression/conditional gen.
            # For unconditional, predict() would return x generated from z=0 (deterministic center)
            # or if we pass prng_key, random sample.
            # Trainer.predict doesn't take prng_key by default (in current signature).
            # But I didn't change signature of predict in this file yet (except adding docstring).
            # I should update predict to handle x=None too if needed.
            
            # For unconditional training metrics (PVE/MSE), comparing generated x to target y doesn't make sense
            # unless it's an autoencoder. Here we are doing flow matching.
            # We are modeling distribution. PVE/MSE against training data is not well-defined for generative model.
            # So we might skip PVE/MSE for unconditional or if x_data is None.
            
            if x_data is not None:
                train_pred = np.array(self.predict(x_viz))
    ...
    def predict(self, x_data: jnp.ndarray, num_steps: int = 20) -> jnp.ndarray:
        """Make predictions (conditional generation)."""
        if self.params is None:
            raise ValueError("Model not initialized.")
        return self.model.predict(self.params, x_data, num_steps, "midpoint", "end_point")

    def predict_latent(self, x_data: jnp.ndarray, num_steps: int = 20, output_type: str = "trajectory") -> jnp.ndarray:
        """Get latent representation (forward flow)."""
        if self.params is None:
            raise ValueError("Model not initialized.")
        # Use 'end_point' if you just want z0, 'trajectory' for full path
        return self.model.predict_latent(self.params, x_data, num_steps, "midpoint", output_type)
    
    def sample(self, num_samples: int, num_steps: int = 20, prng_key: Optional[jr.PRNGKey] = None) -> jnp.ndarray:
        """Unconditional sampling (generation)."""
        if self.params is None:
            raise ValueError("Model not initialized.")
        ...
        # Get output shape from config (this is the shape of generated samples)
        # In regression config, output_shape is the target Y.
        # In generation config (unconditional), input_shape might be empty, output_shape is the data.
        # We should rely on what the model expects. FlowModel.sample uses self.z_shape.
        # We just need to pass batch_shape.
        
        batch_shape = (num_samples,)
        return self.model.sample(self.params, prng_key, batch_shape, num_steps, "midpoint", "end_point")

    def conditional_sample(self, x_data: jnp.ndarray, num_steps: int = 20, prng_key: Optional[jr.PRNGKey] = None) -> jnp.ndarray:
        """Conditional sampling (generation with stochasticity if applicable).
        For deterministic flows (Flow Matching), this is equivalent to predict().
        """
        if self.params is None:
            raise ValueError("Model not initialized.")
        ...
        return self.model.predict(self.params, x_data, num_steps, "midpoint", "end_point", prng_key=prng_key)
