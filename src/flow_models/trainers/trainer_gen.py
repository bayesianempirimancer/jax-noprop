...
from src.flow_models.config_all import Config
...
class GenerationTrainer:
    ...
    def conditional_generate_latent(self, cond_y: jnp.ndarray, num_steps: int = 20, integration_method: str = 'midpoint', output_type: str = "end_point", prng_key: Optional[jr.PRNGKey] = None) -> jnp.ndarray:
        """Generate latent trajectories conditioned on y, skipping the decoder.
        
        This method returns the latent representation z directly without decoding,
        allowing visualization of latent space trajectories.
        
        Args:
            cond_y: Conditional input [batch_shape + y_shape]
            ...
            prng_key: Optional PRNG key for generative mode
            
        Returns:
            Latent representation [batch_shape + z_shape] or [num_steps, batch_shape + z_shape] if trajectory
        """
        if self.params is None:
            raise ValueError("Model not initialized.")
        if self.unconditional:
        ...
        if prng_key is None:
            self.rng, prng_key = jr.split(self.rng)
        return self.model.predict_latent(self.params, cond_y, num_steps, integration_method, output_type, prng_key=prng_key)
