#!/usr/bin/env python3
"""
Training script for regression/classification (x -> y).

This script trains the selected model with forward mapping (inputs=x, targets=y)
...
def main():
    parser = argparse.ArgumentParser(
        description='Regression/classification training (x -> y) with config file support'
    )
    ...
    # Priority: command line args > config file > hardcoded defaults
    # Check config.main dict first, then top-level config attributes, then defaults
    ...
    args.save_dir = get_save_directory(args.save_dir, 'reg', args.model_type)
