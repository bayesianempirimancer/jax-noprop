...
from src.flow_models.crns.crn_seq import TransformerSeq2SeqConditionalResnet
...
def create_conditional_resnet(config_dict: Union[Dict[str, Any], FrozenDict], latent_shape: Tuple[int, ...], input_shape: Tuple[int, ...], output_shape: Optional[Tuple[int, ...]] = None) -> nn.Module:
    ...
    # Update with additional parameters from config_dict (excluding model_type, network_type, and input_shape)
    # input_shape is already set correctly from the function parameter, so we don't want to overwrite it
    # with potentially "NA" or None values from the config_dict
    resnet_config = {k: v for k, v in final_config.items() if k not in ["model_type", "network_type", "input_shape"]}
    ...
    if model_type == "vanilla":
        ...
        from src.flow_models.crns.crn_grads import create_gradient_wrapper
