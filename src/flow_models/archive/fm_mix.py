...
class VAE_flow_mix(nn.Module):
    ...
    @nn.compact
    def compute_gmm_loss(self, z_target: jnp.ndarray, training: bool = True) -> jnp.ndarray:
        """Compute GMM loss for z_target using flow planner.
        
        Flattens z_target to vector format for flow planner.
        """
        # Flatten z_target to [batch, z_dim] for flow planner
        z_target_flat = self._flatten_z(z_target)
        
        # Compute GMM loss from flow planner (expects [batch, latent_dim])
        gmm_loss = self.flow_planner.compute_gmm_loss(z_target_flat, training=training)
    ...
    def loss(self, params: dict, x: Optional[jnp.ndarray], y: jnp.ndarray, key: jr.PRNGKey, training: bool = True, x_mask: Optional[jnp.ndarray] = None) -> Tuple[jnp.ndarray, dict]:
        ...
        z_0 = z_0 + jr.normal(z_0_noise_key, z_0.shape)
        ...
        # Compute Losses (SNR for linear paths is a constant)
        ...
        if use_snr_weight:
            target_snr_weight = 1.0/(1.0-t)**2 # assumes linear noise schedule, i.e. alpha = t.
            if normalize_snr_weight:
                target_snr_weight_mean = jnp.mean(target_snr_weight)
                target_snr_weight = target_snr_weight / (target_snr_weight_mean + 1e-8)
        else:
            target_snr_weight = 1.0
            # Normalize SNR weights by their mean if normalize_snr_weight is True
            
        recon_loss = jnp.mean(target_snr_weight * recon_loss)  # Average over batch dimension
    ...
    @partial(jax.jit, static_argnums=(0, 3, 4, 5))  # self, num_steps, integration_method, output_type are static arguments
    def predict_latent(self, params: dict, x: jnp.ndarray, num_steps: int = 20, integration_method: str = "euler", output_type: str = "end_point", prng_key: jr.PRNGKey = None) -> jnp.ndarray:
        """
        ...
        Args:
            ...
            x: Input data [batch_shape + x_shape] (for conditional generation, this is the condition y)
            num_steps: Number of integration steps (default: 20)
            integration_method: Integration method ("euler", "midpoint", etc.) (default: "euler")
            output_type: Type of output ("end_point" for final latent, "trajectory" for full trajectory)
            prng_key: Optional PRNG key for generative mode. If provided, samples z_0 using flow planner.
        ...
        z = self._unflatten_z(z)
        # Return latent z directly without decoding
        return z
