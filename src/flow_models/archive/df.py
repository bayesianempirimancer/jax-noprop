...
class VAE_flow(nn.Module):
    ...
    @nn.compact
    def lazy_flow_from_noise(self, z: jnp.ndarray, predicted_noise: jnp.ndarray, alpha_t: jnp.ndarray, gamma_prime_t: jnp.ndarray) -> jnp.ndarray:
        """Lazy routine for flow from noise - no params needed."""
        return self.noise_schedule.lazy_flow_from_noise(z, predicted_noise, alpha_t, gamma_prime_t)
    
    @nn.compact
    def lazy_noise_snr(self, alpha_t: jnp.ndarray, gamma_prime_t: jnp.ndarray) -> jnp.ndarray:
        """Lazy routine for noise SNR - no params needed."""
        return self.noise_schedule.lazy_noise_snr(alpha_t, gamma_prime_t)
    ...
    def dz_dt(self, z: jnp.ndarray, x: jnp.ndarray, t: jnp.ndarray, training: bool = False) -> jnp.ndarray:
        ...
        return self.noise_schedule.lazy_flow_from_noise(z, predicted_noise, alpha, gamma_prime)
    ...
    def loss(self, params: dict, x: Optional[jnp.ndarray], y: jnp.ndarray, key: jr.PRNGKey, training: bool = True) -> Tuple[jnp.ndarray, dict]:
        ...
        dz_dt = self.apply(params, z_t, predicted_noise, alpha_t, gamma_prime_t, method='lazy_flow_from_noise')
        ...
        if use_snr_weight:
            snr_weight = self.apply(params, alpha_t, gamma_prime_t, method='lazy_noise_snr')
        ...
        if use_snr_weight:
            recon_snr = self.apply(params, alpha_t, gamma_prime_t, method='lazy_target_snr')
            recon_snr = jnp.squeeze(recon_snr, axis=tuple(range(-self.z_ndims, 0)))
