...
class VQVAETrainer:
    ...
    def _plot_training_progress(self, history: Dict[str, list], save_path: Path):
        ...
        if len(history['val_losses']) > 0:
            ...
            # Only use as many validation points as we have
        ...
        # VQ loss
        axes[1, 0].plot(epochs, history['train_vq_losses'], label='Train', alpha=0.7)
        if len(history['val_vq_losses']) > 0 and len(val_epochs) == len(history['val_vq_losses']):
            axes[1, 0].plot(val_epochs, history['val_vq_losses'], label='Val', marker='o', alpha=0.7)
        ...
        # Percent Variance Explained by dimension (replaces commitment loss)
        axes[1, 1].set_xlabel('Epoch')
        axes[1, 1].set_ylabel('% Variance Explained')
        axes[1, 1].set_title('Percent Variance Explained by Dimension')
        axes[1, 1].grid(True, alpha=0.3)
        
        # Plot PVE for each dimension
        if len(history.get('train_pve_by_dim', [])) > 0:
            # Get number of dimensions from first entry
            ...
            if len(first_pve) > 0:
                ...
                for dim in range(num_dims):
                    ...
                    # Plot training PVE
                    dim_label = f'Dim {dim} (Train)'
                    if dim == 0:
                        dim_label = 'Price (Train)'  # Assuming first dim is price
                    elif dim == 1:
                        dim_label = 'Volume (Train)'  # Assuming second dim is volume
                    axes[1, 1].plot(epochs[:len(train_pve_dim)], train_pve_dim, 
                                   label=dim_label, alpha=0.7, linestyle='-')
                
                # Plot validation PVE if available
                if len(history.get('val_pve_by_dim', [])) > 0 and len(val_epochs) > 0:
                    for dim in range(num_dims):
                        val_pve_dim = []
                        ...
                        # Plot validation PVE at validation epochs
                        dim_label = f'Dim {dim} (Val)'
                        if dim == 0:
                            dim_label = 'Price (Val)'
                        elif dim == 1:
                            dim_label = 'Volume (Val)'
                        if len(val_pve_dim) == len(val_epochs):
                            axes[1, 1].plot(val_epochs, val_pve_dim, 
                                           label=dim_label, marker='o', alpha=0.7, linestyle='--')
        
        axes[1, 1].legend()
        # Allow negative PVE (indicates worse than mean prediction)
        # Set ylim to show negative values if they exist
        if len(history.get('train_pve_by_dim', [])) > 0:
            all_pve_values = []
            ...
            if all_pve_values:
                ...
                axes[1, 1].set_ylim(bottom=pve_min - 0.1 * y_range, top=pve_max + 0.1 * y_range)
            else:
                axes[1, 1].set_ylim(bottom=-50, top=100)  # Default range
        else:
            axes[1, 1].set_ylim(bottom=-50, top=100)  # Default range
