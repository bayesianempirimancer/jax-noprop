...
def plot_latent_trajectories(
    ...
    # Generate trajectories (no prng_key needed for MLE predictions)
    trajectories = []
    
    integration_method = "midpoint" if model_type == "ct" else "euler"
    
    for i in range(n_samples):
        # Always use predict() for deterministic MLE trajectories (no prng_key)
        if unconditional:
            # For unconditional, pass None as cond_x
            traj = model.predict(
                params,
                None,  # No condition for unconditional
                num_steps=num_steps,
                integration_method=integration_method,
                output_type="trajectory",
                prng_key=None  # No prng_key for MLE
            )
        else:
